...
export default function ClientServiceDetail() {
    ...
    const handleSaveEdit = async () => {
        ...
        if (editData.serviceStartDate && editData.serviceStartDate !== original.serviceStartDate) {
        ...
        if (editData.serviceStatusChangeDate && editData.serviceStatusChangeDate !== original.serviceStatusChangeDate) {
    ...
    const handleDelete = async (serviceId) => {
        if (!window.confirm("Are you sure you want to de-assign this service?")) return;
                            ...
                            >
                                <div className={styles.topRow}>
                                    <div className={styles.leftSide}>
                                        <span className={styles.serviceName}>
                                            {service.serviceName} [Duration: {service.serviceDuration} days]
                                        </span>
                                        {!service.deleted && (
                                            <span
                                ...
                                <div className={styles.dateRow}>
                                    {!service.deleted && service.updatedServiceDuration ? (
                                        <span>Please note that your service duration is extended {service.updatedServiceDuration} days by admin.</span>
                                    ) : (
                                        ''
                                    )}
                                </div>
                                        ...
                                        <>
                                            <span>
                                                {service.serviceStartDate
                                                    ? new Date(service.serviceStartDate).toLocaleDateString("en-GB")
                                                    : "-"}
                                            </span>
                                            <span className={styles.arrow}>→</span>
                                            <span>
                                ...
                                {isCurrentEditing && (
                                    <div className={styles.actionRow}>
                                        ...
                                        <button className={styles.cancleBtn} onClick={handleCancelEdit}>
                                ...
                                {!isCurrentEditing && service.serviceStatusChangeDate && (
                                    <div className={styles.bottomRow}>
                                        <span className={styles.statusText}>
                                            Status changed on:{" "}
                                            {new Date(service.serviceStatusChangeDate).toLocaleDateString('en-GB')}
                                        </span>
                                    </div>
                                )}
                                ...
                                {!isCurrentEditing && service.deassignDate && (
                                    <div className={styles.bottomRow}>
                                        <span className={styles.statusText}>
                                            ...
                                            {new Date(service.deassignDate).toLocaleDateString('en-GB')}
                                        </span>
                                    </div>
                                )}
                            </div>
