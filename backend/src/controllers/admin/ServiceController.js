...
exports.assignService = async (req, res) => {
    ...
    const isAllServiceCompleted = await ClientService.checkAllServiceCompleted(client, serviceId);

    if(!isAllServiceCompleted) {
        throw new ValidationError(ValidationMsg.ServiceIsNotCompleted);
    }

    const isServiceRunning = await ClientService.checkIsServiceRunning(client, serviceId);
...
exports.deAssignService = async (req) => {
    ...
    const isInternalServiceExists = await ClientService.checkInternalServiceExists(client, serviceId);
