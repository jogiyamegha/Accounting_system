...
exports.deAssignService = async (req) => {
  ...
  const isServiceAssign = await ClientService.checkIsServiceAssign(
    client,
    serviceId
  );
  ...
  const isServiceCompleted = await ClientService.checkServiceCompletedOrNot(
    client,
    serviceId
  );
  ...
  // await Email.sendServiceDeAssignMail(
  //   client[TableFields.name_],
  //   client[TableFields.email],
  //   service[TableFields.serviceName]
  // );
