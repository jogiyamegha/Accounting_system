...
exports.sendServiceDeAssignMail = async (name, emailId, serviceName) => {
  const invitationTemplate = fs
    .readFileSync(
      path.join(customerViewDirPath, "client", "service-deassign.hbs")
    )
    .toString();
  ...
  const template = Handlebars.compile(invitationTemplate);
  try {
    await sendEmail(emailId, GeneralMessages.ServiceDeAssign, template(data));
  } catch (e) {
    console.log(e);
  }
};

exports.sendServiceStatusMail = async (name, emailId, status) => {
  const invitationTemplate = fs
    .readFileSync(
      path.join(customerViewDirPath, "client", "service-statusChange.hbs")
    )
    .toString();
  let data = {
    name: name,
  ...
  const template = Handlebars.compile(invitationTemplate);
  try {
    await sendEmail(emailId, GeneralMessages.ServiceStatus, template(data));
  } catch (e) {
    console.log(e);
  }
};
...
exports.sendServiceRenewalMail = async (name, emailId, serviceName) => {
  const invitationTemplate = fs
    .readFileSync(
      path.join(customerViewDirPath, "client", "service-renewal.hbs")
    )
    .toString();
